...
- (WindowDelegate*)initWithWindow:(ikun_gui_app::Window_mac*)initWindow;

@end

@interface MainView : NSView

- (MainView*)initWithWindow:(ikun_gui_app::Window_mac*)initWindow;

@end

///////////////////////////////////////////////////////////////////////////////

using ikun_gui_app::Window;
...
void Window_mac::closeWindow() {
  if (nil != fWindow) {
    ...
    if (ikun_gui_app::Window_mac::gWindowMap.count() < 1) {
...
@implementation WindowDelegate {
  ikun_gui_app::Window_mac* fWindow;
}
...
- (void)updateTrackingAreas {
  ...
  fTrackingArea = [[NSTrackingArea alloc] initWithRect:[self bounds] options:options owner:self userInfo:nil];
...
- (skui::ModifierKey)updateModifierKeys:(NSEvent*)event {
  ...
  for (const ModMap& cur : {ModMap{skui::ModifierKey::kCommand, skui::Key::kSuper}, ModMap{skui::ModifierKey::kShift, skui::Key::kShift},
                            ModMap{skui::ModifierKey::kControl, skui::Key::kCtrl}, ModMap{skui::ModifierKey::kOption, skui::Key::kOption}}) {
    if (Any(changed & cur.modifier)) {
      const skui::InputState state = Any(modifiers & cur.modifier) ? skui::InputState::kDown : skui::InputState::kUp;
...
- (void)mouseDown:(NSEvent*)event {
  ...
  fWindow->onMouse(pos.x * backingScaleFactor, (rect.size.height - pos.y) * backingScaleFactor, skui::InputState::kDown, modifiers);
}
...
- (void)mouseUp:(NSEvent*)event {
  ...
  const NSPoint pos = [event locationInWindow];
  const NSRect rect = [view frame];
  fWindow->onMouse(pos.x * backingScaleFactor, (rect.size.height - pos.y) * backingScaleFactor, skui::InputState::kUp, modifiers);
}
...
- (void)mouseMoved:(NSEvent*)event {
  ...
  const NSPoint pos = [event locationInWindow];
  const NSRect rect = [view frame];
  fWindow->onMouse(pos.x * backingScaleFactor, (rect.size.height - pos.y) * backingScaleFactor, skui::InputState::kMove, modifiers);
}
