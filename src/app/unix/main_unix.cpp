...
int main(int argc, char** argv) {
  ...
  ikun_gui_app::Application* app = ikun_gui_app::Application::Create(argc, argv, (void*)display);
  ...
  while (!done) {
    ...
    if (int count = XPending(display)) {
      ...
      THashSet<ikun_gui_app::Window_unix*> pendingWindows;
      while (count-- && !done) {
        XEvent event;
        XNextEvent(display, &event);

        ikun_gui_app::Window_unix* win = ikun_gui_app::Window_unix::gWindowMap.find(event.xany.window);
        if (!win) {
          continue;
        }
      ...
      pendingWindows.foreach ([](ikun_gui_app::Window_unix* win) {
        win->finishResize();
        win->finishPaint();
      });
    } else {
